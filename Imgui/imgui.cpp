...
{
	//MyState
	static D3DXVECTOR3 imgui_position_[10];
	static D3DXVECTOR3 imgui_scale_[10];
	static D3DXVECTOR3 imgui_rotation_[10];

	static D3DXVECTOR3 setpos[10];
	static D3DXVECTOR3 setscale[10];
	static D3DXVECTOR3 setrot[10];

	bool SaveFlag = false;

	IMGUI_API void GetFPS()
	{
		if (ImGui::Begin("FPS"))
		{
	...
	IMGUI_API void GetTargetInfomation(ImVec2 pos, ImVec2 size, ImVec4 activecolor, ImVec4 bgcolor, const char* name, D3DXMATRIX MtxTarget, D3DXVECTOR3 TargetPosition, D3DXVECTOR3 TargetRotation, D3DXVECTOR3 TargetScale)
	{
		static int number = 0;
		ImGui::SetNextWindowSize(ImVec2(size.x, size.y), ImGuiSetCond_Once);
		ImGui::SetNextWindowPos(ImVec2(pos.x, pos.y), ImGuiSetCond_Once);
		ImGui::PushStyleColor(ImGuiCol_TitleBg, ImVec4(bgcolor.x, bgcolor.y, bgcolor.z, bgcolor.w));
		ImGui::PushStyleColor(ImGuiCol_TitleBgActive, ImVec4(activecolor.x, activecolor.y, activecolor.z, activecolor.w));
		...
		if (ImGui::Begin(name, nullptr, ImGuiWindowFlags_MenuBar))
		{
			...
			if (number == 0)
			{
				ImGui::Text("m[1]   m_11:%.3f m_12:%.3f m_13:%.3f m_14:%.3f ", MtxTarget._11, MtxTarget._12, MtxTarget._13, MtxTarget._14);
				ImGui::Text("m[2]   m_21:%.3f m_22:%.3f m_23:%.3f m_24:%.3f ", MtxTarget._21, MtxTarget._22, MtxTarget._23, MtxTarget._24);
				ImGui::Text("m[2]   m_31:%.3f m_32:%.3f m_33:%.3f m_34:%.3f ", MtxTarget._31, MtxTarget._32, MtxTarget._33, MtxTarget._34);
				ImGui::Text("m[2]   m_41:%.3f m_42:%.3f m_43:%.3f m_44:%.3f ", MtxTarget._41, MtxTarget._42, MtxTarget._43, MtxTarget._44);
			}
			if(number == 1)
			{
				ImGui::Text("Position_X:%.3f", TargetPosition.x);
				ImGui::Text("Position_Y:%.3f", TargetPosition.y);
				ImGui::Text("Position_Z:%.3f", TargetPosition.z);
			}
			if (number == 2)
			{
				ImGui::Text("Rotation_X:%.3f", TargetRotation.x);
				ImGui::Text("Rotation_Y:%.3f", TargetRotation.y);
				ImGui::Text("Rotation_Z:%.3f", TargetRotation.z);
			}
			if (number == 3)
			{
				ImGui::Text("Scale_X:%.3f", TargetScale.x);
				ImGui::Text("Scale_Y:%.3f", TargetScale.y);
				ImGui::Text("Scale_Z:%.3f", TargetScale.z);
			}
		}
		ImGui::End();
		ImGui::PopStyleColor();
	...
	IMGUI_API void InitModelData(int number, D3DXVECTOR3 position, D3DXVECTOR3 rotation, D3DXVECTOR3 scale)
	{
		imgui_position_[number] = position;
		imgui_rotation_[number] = rotation;
		imgui_scale_[number] = scale;
	}

	IMGUI_API void SaveModelData()
	{
		FILE* fp;
		if ((fp = fopen("ModelAnimation.csv", "a")) != NULL)
		{
			for (int BoneFirst = 0; BoneFirst < 10; BoneFirst++)
			{
				fprintf(fp, "%f,%f,%f\n", setpos[BoneFirst].x, setpos[BoneFirst].y, setpos[BoneFirst].z);
				fprintf(fp, "%f,%f,%f\n", setrot[BoneFirst].x, setrot[BoneFirst].y, setrot[BoneFirst].z);
				fprintf(fp, "%f,%f,%f\n", setscale[BoneFirst].x, setscale[BoneFirst].y, setscale[BoneFirst].z);
			}
		}
		fclose(fp);
	}

	IMGUI_API void SaveDataClear()
	{
		FILE* fp;
		if ((fp = fopen("ModelAnimation.csv", "w")) != NULL)
		{
			for (int i = 0; i < 2; i++)
			{
				for (int BoneFirst = 0; BoneFirst < 10; BoneFirst++)
				{
					fprintf(fp, "%f,%f,%f\n", imgui_position_[BoneFirst].x, imgui_position_[BoneFirst].y, imgui_position_[BoneFirst].z);
					fprintf(fp, "%f,%f,%f\n", imgui_rotation_[BoneFirst].x, imgui_rotation_[BoneFirst].y, imgui_rotation_[BoneFirst].z);
					fprintf(fp, "%f,%f,%f\n", imgui_scale_[BoneFirst].x, imgui_scale_[BoneFirst].y, imgui_scale_[BoneFirst].z);
				}
			}
		}
		fclose(fp);
	...
	IMGUI_API void ToolFlag(int & toolflag)
	{
		toolflag *= -1;
	}

	IMGUI_API void SetModelInfo(int number,const char * name, D3DXVECTOR3 & position, D3DXVECTOR3 & rotation, D3DXVECTOR3 & scale)
	{
		...
		ImGui::Begin(name);
			if (number == 0)
			{
				//Save 動かしたモデルの状態を追加保存していく
				//SaveDataReSet 2フレーム用Tボーンの位置を保存
				//SaveDataDelete 保存したデータの消去
				//if (ImGui::Button("ReSet"))
				//{
				//	for (int i = 0; i < 10; i++)
				//	{
				//		position = imgui_position_[i];
				//		rotation = imgui_rotation_[i];
				//		scale = imgui_scale_[i];
				//	}
				//}
				...
				if (ImGui::Button("SaveDataReSet"))
				{
					SaveDataClear();
				}
				if (ImGui::Button("SaveDataDelete"))
				{
					SaveDataDelete();
				}
				...
				if (ImGui::TreeNode("BODY"))
				{
					...
					setrot[0] = rotation;
					setscale[0] = scale;
					ImGui::SliderFloat3("position", position, -10.0f, 10.0f, "%.2f");
					ImGui::SliderFloat3("rotation", rotation, D3DXToRadian(-360), D3DXToRadian(360), "%.2f");
					ImGui::SliderFloat3("scale", scale, 0.0f, 10.0f, "%.2f");
					ImGui::TreePop();
				}
			}
			if (number == 1)
			...
			if (number == 5)
			{
				ImGui::SetNextTreeNodeOpen(true, ImGuiSetCond_Once);
				ImGui::PushItemWidth(500);
				if (ImGui::TreeNode("RIGHTFOREARM"))
				{
					setpos[5] = position;
					setrot[5] = rotation;
					setscale[5] = scale;
			...
			if (number == 7)
			{
				ImGui::SetNextTreeNodeOpen(true, ImGuiSetCond_Once);
				ImGui::PushItemWidth(500);
				if (ImGui::TreeNode("LEFTLOWERLEG"))
				{
					setpos[7] = position;
					setrot[7] = rotation;
					setscale[7] = scale;
			...
			if (number == 9)
			{
				ImGui::SetNextTreeNodeOpen(true, ImGuiSetCond_Once);
				ImGui::PushItemWidth(500);
				if (ImGui::TreeNode("RIGHTLOWERLEG"))
				{
					setpos[9] = position;
					setrot[9] = rotation;
					setscale[9] = scale;
	...
	IMGUI_API void GetMatrixInfomation(const char * name, D3DXMATRIX MtxTarget)
	{
		if (ImGui::Begin(name))
		{
			ImGui::Text("m[1]   m_11:%.3f m_12:%.3f m_13:%.3f m_14:%.3f ", MtxTarget._11, MtxTarget._12, MtxTarget._13, MtxTarget._14);
	...
	IMGUI_API void GetPositionInfomation(const char * name, D3DXVECTOR3 TargetPosition)
	{
		if (ImGui::Begin(name))
		{
			ImGui::Text("Position_X:%.3f", TargetPosition.x);
	...
	IMGUI_API void GetRotationInfomation(const char * name, D3DXVECTOR3 TargetRotation)
	{
		if (ImGui::Begin(name))
		{
			ImGui::Text("Rotation_X:%.3f", TargetRotation.x);
	...
	IMGUI_API void GetScaleInfomation(const char * name, D3DXVECTOR3 TargetScale)
	{
		if (ImGui::Begin(name))
		{
			ImGui::Text("Scale_X:%.3f", TargetScale.x);
	...
	IMGUI_API void GetKeyFrameAnimation(const char * name, float TargetKeyFrameAnimation)
	{
		if (ImGui::Begin(name))
		{
